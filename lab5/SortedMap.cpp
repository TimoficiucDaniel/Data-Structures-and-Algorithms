...
///BC = WC = AVG: theta(height_of_subtree) <==>theta(n)
{
    bool found = false;
    int old = this->search(k);
    if (old != NULL_TVALUE){
...
void SortedMap::replace(TKey k, TValue oldValue , TValue newValue) {
    ...
    while (current != NULL_TVALUE){
        if (this->bst.nodes[current].info.first == k){
            if(this->bst.nodes[current].info.second == oldValue) {
                this->bst.nodes[current].info.second = newValue;
                return;
            }
            else
                return;
